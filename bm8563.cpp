...
{
    return i2c_read(BM5863_ADDR, 0x09) & 0x7f;
}

void bm8563::set_minute_alarm(uint8_t minute)
{
    i2c_write(BM5863_ADDR, 0x09, minute & 0x7f);
}
...
{
    return i2c_read(BM5863_ADDR, 0x0A) & 0x3f;
}

void bm8563::set_hour_alarm(uint8_t hour)
{
    i2c_write(BM5863_ADDR, 0x0A, hour & 0x3f);
}
...
{
    return i2c_read(BM5863_ADDR, 0x0B) & 0x3f;
}

void bm8563::set_day_alarm(uint8_t minute)
{
    i2c_write(BM5863_ADDR, 0x0B, minute & 0x3f);
}
...
{
    return i2c_read(BM5863_ADDR, 0x0C) & 0x07;
}
